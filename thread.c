...
Checks if the NEW_PRIORITY is less than that of in ready list (dependeing on it is sorted)*/
void
thread_set_priority (int new_priority) 
{
  if (thread_current()->priority == thread_current()->priority_donation)
  {
    thread_current()->priority = new_priority;
    thread_current()->priority_donation = new_priority;
  }
  else
    thread_current()->priority_donation = new_priority;
  ...
  {
    struct list_elem *elem_ready = list_begin (&ready_list);
    struct thread *t = list_entry (elem_ready, struct thread, elem);

    if (new_priority < t->priority && t != NULL)
      thread_yield();
  }
}
...
void
thread_priority_donation(struct thread *donated_thread, int new_priority)
{
  donated_thread->priority = new_priority;

  if (!list_empty (&ready_list) && donated_thread == thread_current())
  {
    struct list_elem *elem_ready = list_begin (&ready_list);
    struct thread *t = list_entry(elem_ready, struct thread, elem);
